...
let set2 = new Set<number>([1,2,3,4,5,6]);
let set1 = new Set<number>([1,2,3,6,7]);
// console.log(intersection(set1,set2))  //--> [ 1, 2, 3, 6 ]
// console.log(subtract(set1,set2));     //--> [7]

type Occurence = {
    str: string; 
    count: number;
}
function getSortedOccurences(array: string[]): Occurence[]{
...
// console.log(arValues); // --> [ 'JAN', 'Feb', 'Mar', 'Apr', '10' ]

type Person = {
    id: number,
    name: string,
    age?: number,  // ? --> defines an optional field
    city?: string
}
...
let person: Person = {id: 100, name: 'Alex', age: 40, city: "Haifa"}
let person2: Person = {id:101, name: 'Felix', age: 31, city: 'TA'}
let persons: Person[] = [];   // HAVE TO  DEFINE ARRAY THIS WAY  !!!!!!!!!
persons.push(person)  ;
